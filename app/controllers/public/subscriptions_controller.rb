class Public::SubscriptionsController < ApplicationController
  ...
  def index
    @subscriptions = current_customer.subscriptions.where(item: @item)
  end
  ...
  def update
    ...
    if @subscription.update(subscription_params)
      redirect_to item_subscriptions_path(@item), notice: "定期購入を更新しました"
    else
      render :edit
    end
  end
  ...
  def cancel
    ...
    redirect_to item_subscriptions_path(@item), notice: "定期購入を解約しました"
  end
